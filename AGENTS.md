...
- `tests/` — `server.pli` fixture (echo/http on `18080`, `/delay` sleeps `2000` via `delay`) + `http_get`/`echo`/`send_recv`/`resolve_dial`/`close_shutdown`/`timeout`/`ephemeral`/`poll`/`nonblocking` (9 e2e)
...
- never change the indentation of the existing code unless explicitly asked to
